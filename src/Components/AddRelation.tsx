...
const AddRelation = ({personList,setRelationship}) => {
  ...
  return (
    ...
    alignItems:"center",width:"60%",margin:"10px auto"}}>
      <Box sx={{ minWidth: 120 }}>
    ...
    <Button sx={{
        width: '25ch',
        height:"100%" ,
      }}variant="contained" onClick = {handleClickAdd}>Add</Button>
  </Stack>
    ...
    <ul className = "relationships">
      {relationships.map((relations,key) => (
        <div key = {key} className = "listContainer">
          ...
          <Button variant="outlined" color = "error" onClick = {() => handleRemoveRelation(key)}>X</Button>
      </div>
      ))}
  </ul>
</div>)
