...
const handleClick = () => {
  ...
  for(let relation of relationships){
    addEdge(personList.indexOf(relation.firstPerson),personList.indexOf(relation.secondPerson));
    addEdge(personList.indexOf(relation.secondPerson),personList.indexOf(relation.firstPerson));
  }
  ...
  const Menulist = personList.map((person,key) => {
    return (<MenuItem key = {key} value={person}>{person}</MenuItem>)
  })
  ...
  <Stack spacing={2} direction="row" sx = {{justifyContent:"center",
    alignItems:"center",width:"60%",margin:"auto"}}>
    <Box sx={{ minWidth: 120 }}>
  ...
  <Button sx={{
      width: '25ch',
      height:"100%" ,
    }}variant="contained" onClick = {handleClick}>Get Degree</Button>
</Stack>
<div className = "error">{error}</div>
{path.length > 0 ? path.map((pt,k) => 
  { return (
    <div key = {k} className="relationContainer">
    {JSON.parse(pt).map((p,key) => {
      return (<span className = "individualPath" key = {key}>{personList[p]}  </span>)
    }
    )}
  </div>
  )}
